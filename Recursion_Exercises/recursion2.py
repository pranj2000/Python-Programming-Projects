...
#  Partner's Name: Vivian Chang

#  Partner's UT EID: vc8969

#  Course Name: CS 313E

#  Unique Number: 50210

#  Date Created: 10/6/19
...
def groupSumClump(start, nums, target):
    ...
    for i in range(start+1, len(nums)):
        if nums[i] == nums[start]:
            total += nums[i]
            cnt += 1
    return groupSumClump(start + cnt, nums, target - total) or groupSumClump(start + cnt, nums, target);
...
def splitArrayHelper(nums, x, split):
    if x == len(nums):
        return (split == 0)
    if splitArrayHelper(nums, x+1, split+nums[x]):
        return True
    else:
        return splitArrayHelper(nums, x + 1, split - nums[x])
...
def splitOdd10Helper(nums, x, s, s2):
    if x >= len(nums):
        return (s % 2 == 1 and s2 % 10 == 0) or (s2 % 2 == 1 and s % 10 == 0)
    else:
        return (splitOdd10Helper(nums, x + 1, s + nums[x], s2)) or (splitOdd10Helper(nums, x + 1, s, s2 + nums[x]))
...
def split53Helper(nums, x, s, s2):
    if (x >= len(nums)):
        return (s == s2)
    if (nums[x] % 5 == 0):
        return split53Helper(nums, x + 1, s + nums[x], s2)
    if (nums[x] % 3 == 0):
        return split53Helper(nums, x + 1, s, s2 + nums[x])
    return split53Helper(nums, x + 1, s + nums[x], s2) or split53Helper(nums, x + 1, s, s2 + nums[x])
